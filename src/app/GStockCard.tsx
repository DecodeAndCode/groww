...
export function GStockCard({topGainer}) {
    return <Card sx={{minWidth: 250}} raised={true} variant={"outlined"}>
        <img
            ...
            alt={topGainer.ticker}
        />
        <Typography align={"center"} variant={"h5"} color={"orange"}>{topGainer.ticker}</Typography>
        <div style={{
            marginLeft: 20
        }}>{"$ " + topGainer.price}</div>
        <div style={{
            marginLeft: 20,
            color: "green"
        }}>{"+ " + topGainer.change_percentage}</div>
        <div style={{
            marginLeft: 20,
            color: "green"
        }}>{"+ " + topGainer.change_amount}</div>
    </Card>
}